...
const Navbar = ({ NavColor = "#ffffff", LogoColor = "black" }) => {
  ...
  return (
    <div
      className={`${styles.container} ${isNavbarVisible ? styles.visible : ""}`}
      style={{ backgroundColor: `${NavColor}` }}
    >
      <div className={styles.nav}>
        <div className={styles.logo}>
          <Link to={"/home"}>
            <Logo width={"200px"} height={"60px"} fill={LogoColor} />
          </Link>
        </div>
        <div className={styles.NavbarLinks}>
          {WOMEN.includes(pathname) ? (
            <Link to="/women">
              <h2>WOMEN</h2>
            </Link>
          ) : null}
          {MEN.includes(pathname) ? (
            <Link to="/men">
              <h2>MEN</h2>
            </Link>
          ) : null}
          {KIDS.includes(pathname) ? (
            <Link to="/kids">
              <h2>KIDS</h2>
            </Link>
          ) : null}
          ...
          {role != "admin" ? (
            FAVORITE.includes(pathname) ? (
              <Link
                to={
                  role == "user"
                    ? MENU_USER.favorites.route || DETAIL_PAGE(pathname)
                    : null
                }
              >
                <h3 title="favorites" onClick={handlerChangeFavorite}>
                  {ICONS.FAVORITE_WHITE}
                </h3>
              </Link>
            ) : null
          ) : null}
          {role != "admin" &&
          (TROLLEY.includes(pathname) || DETAIL_PAGE(pathname)) ? (
            <h3>
              {" "}
      ...
      <div className={`${styles.BackHome} ${styles.NavbarLinks}`}>
        {HOME.includes(pathname) || DETAIL_PAGE(pathname) ? (
          <Link to={SESSION_NOT_COOKIE}>
            <h2>{ICONS.ARROW_LEFT_BLACK}BACK TO HOME</h2>
          </Link>
        ) : null}
      </div>
    </div>
